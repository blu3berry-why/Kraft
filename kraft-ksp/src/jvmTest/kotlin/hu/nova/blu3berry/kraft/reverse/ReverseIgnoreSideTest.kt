    ...
    fun `SOURCE direction activates in reverse and skips in forward`() {
        val source = SourceFile.kotlin(
            ...
            @hu.nova.blu3berry.kraft.config.MapConfig(
                ...
                ignoredMappings = [
                    hu.nova.blu3berry.kraft.config.MapIgnoreField("extra", direction = hu.nova.blu3berry.kraft.config.IgnoreSide.TARGET),
                    hu.nova.blu3berry.kraft.config.MapIgnoreField("internalNote", direction = hu.nova.blu3berry.kraft.config.IgnoreSide.SOURCE)
                ]
            )
            object UserMapper
            """
    ...
    fun `BOTH direction applies in both forward and reverse`() {
        val source = SourceFile.kotlin(
            ...
            @hu.nova.blu3berry.kraft.config.MapConfig(
                ...
                target = UserDto::class,
                ignoredMappings = [
                    hu.nova.blu3berry.kraft.config.MapIgnoreField("temp", direction = hu.nova.blu3berry.kraft.config.IgnoreSide.BOTH)
                ]
            )
            object UserMapper
            """
