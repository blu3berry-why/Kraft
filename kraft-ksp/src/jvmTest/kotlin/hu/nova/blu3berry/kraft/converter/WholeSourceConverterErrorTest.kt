...
class WholeSourceConverterErrorTest {
    ...
    fun `wrong parameter type emits error mentioning source class`() {
        val source = SourceFile.kotlin(
            ...
            object SrcMapper {
                @hu.nova.blu3berry.kraft.config.MapUsing(target = "text")
    ...
    fun `duplicate whole-source converters for same target emits error`() {
        val source = SourceFile.kotlin(
            ...
            object SrcMapper {
                @hu.nova.blu3berry.kraft.config.MapUsing(target = "combined")
    ...
    fun `unknown target property emits error`() {
        val source = SourceFile.kotlin(
            ...
            object SrcMapper {
                @hu.nova.blu3berry.kraft.config.MapUsing(target = "nonexistent")
