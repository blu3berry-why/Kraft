...
class PropertySourceConverterErrorTest {
    ...
    fun `unknown source property emits error`() {
        val source = SourceFile.kotlin(
            ...
            object SrcMapper {
                @hu.nova.blu3berry.kraft.config.MapUsing(source = "nonexistent", target = "text")
    ...
    fun `unknown target property emits error`() {
        val source = SourceFile.kotlin(
            ...
            object SrcMapper {
                @hu.nova.blu3berry.kraft.config.MapUsing(source = "value", target = "nonexistent")
                fun convert(v: Int): String = v.toString()
            }
            """
        )
    ...
    fun `blank target emits error`() {
        val source = SourceFile.kotlin(
            ...
            object SrcMapper {
                @hu.nova.blu3berry.kraft.config.MapUsing(source = "value", target = "")
                fun convert(v: Int): String = v.toString()
            }
            """
        )
