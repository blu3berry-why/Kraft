    ...
    fun `@MapField renames are correctly inverted in reverse`() {
        val source = SourceFile.kotlin(
            ...
            data class UserDto(
                ...
                val id: Int,
                @com.blu3berry.kraft.mapping.MapField(counterPartName = "fullName")
                val name: String
            )
            """
        )
    ...
    fun `config-level @FieldMapping renames are correctly inverted in reverse`() {
        val source = SourceFile.kotlin(
            ...
            @com.blu3berry.kraft.config.MapConfig(
                ...
                fieldMappings = [
                    com.blu3berry.kraft.config.FieldMapping(source = "userId", target = "id"),
                    com.blu3berry.kraft.config.FieldMapping(source = "fullName", target = "name")
                ]
            )
            object UserMapper
            """
