package com.blu3berry.kraft.propertyresolver.rules
...
class ClassOverrideRuleTest {
    ...
    fun `match - @MapField renames source property in generated code`() {
        val source = SourceFile.kotlin(
            ...
            @com.blu3berry.kraft.mapping.MapFrom(UserSource::class)
            data class UserDto(
                @com.blu3berry.kraft.mapping.MapField(counterPartName = "userId")
                val id: Int,
                val fullName: String
            )
            """
    ...
    fun `error - @MapField referencing unknown source property emits error`() {
        val source = SourceFile.kotlin(
            ...
            data class UserDto(
                val name: String,
                @com.blu3berry.kraft.mapping.MapField(counterPartName = "nonexistent")
                val id: Int
            )
            """
        )
    ...
    fun `error - @MapField with type mismatch between source and target emits error`() {
        val source = SourceFile.kotlin(
            ...
            @com.blu3berry.kraft.mapping.MapFrom(UserSource::class)
            data class UserDto(
                @com.blu3berry.kraft.mapping.MapField(counterPartName = "age")
                val age: Int
            )
            """
        )
